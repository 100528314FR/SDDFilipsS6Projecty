...
public class grid {
    ...
    // various required variables, size of field, name and difficulty are determined in the
    // previous window
    int rows = menu.rows;
    ...
    // used for when you click on a mine and for revealing a revealed tiles
    // neighbors when clicked again
    int[][] clicked = new int[rows][columns];

    public grid() {
        run = true;
        // start a new thread for timer in background so that the gmae can be played
        // simultaniously
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                // timer only stops when game ends (win or lose), or gmae is reset with reset
                // button
                while (run == true) {
                    ...
                    // every second, each image is updated to the values found above
                    try {
        ...
        reset.addActionListener(new ActionListener() {
            // the reset button stops the current timer, closes all windows and re runs the
            // main method, creating a new window and minefield
    ...
    public int[][] mineGen() {
        ...
        // return the 2d array for use in behaviour method
        return minef;
    }
                    ...
                    for (int a = i - 1; a < i + 2; a++) {
                        for (int b = j - 1; b < j + 2; b++) {
                            try {
                                ...
                                // if its counting a tile on the edge and tries to count a non-existant
                                // neighbor, just skip it
                                continue;
                            }
                        }
                    }
    ...
    public void reveal(int xPos, int yPos) {
        ...
        if (count[xPos][yPos] != 0) {
            // reveal the lsat tile
            revealed[xPos][yPos] = true;
            // set it to have been clicked (for nieghbor revealing)
            clicked[xPos][yPos] = 1;
            // end recursion
            return;
        } else if (count[xPos][yPos] == 0) {
            // for all 8 tiles surrounding current one
            for (int a = xPos - 1; a < xPos + 2; a++) {
                for (int b = yPos - 1; b < yPos + 2; b++) {
                    try {
                        ...
                        // try catch is used for when a tiles on one of the edges is being dealt with,
                        // if the current enighbor is outsize of the array, continue to the next one
                        // instead of crashing
    ...
    // method for printing tiles mainly used for recursive method, when lots of
    // tiles are being revealed at once
    ...
    // generates a grid of buttons, and determines the behaviour of each based on
    // the 2d arrays of properties, some of which are already known (count and minef)
    // others are changed by what the player does
    public void buttonBehaviour() {
        // calls the mine generation and count to be used to determine the behaviour of
        // buttons
        mineGen();
        counts();
                    ...
                    buttons[i][j].addMouseListener(new MouseAdapter() {
                        ...
                        public void mouseClicked(MouseEvent me) {
                            ...
                            if (SwingUtilities.isRightMouseButton(me)) {
                                // a counter of flagged mines (only increments when a mine is flagged, not empty
                                // tiles) used for win condition
                                flaggedM++;
                                ...
                                // for use when the very first click would reveal a bomb and lose the game, with
                                // no information for the platyer, this would be unfair
                                // if this is the first click of the game and a bomb has been clicked
                                if (firstClick == 0) {
                                    // change the bomb to an empty tile
                                    minef[I][J] = 0;
                                    count[I][J] = 0;
                                    // recount numbers, for the tiles around the ex-bomb and the new empty tile
                                    // itself if it had bomb neighbors
                                    counts();
                                    ...
                                    if (count[I][J] > 0) {
                                        for (int a = I - 1; a < I + 2; a++) {
                                            for (int b = J - 1; b < J + 2; b++) {
                                                ...
                                                // this gives the player a few empty tiles to start solving with,
                                                // instead of just one tile
                                                try {
                                                    if (minef[a][b] == 0 && count[a][b] > 0) {
                                                        clicked[a][b] = 1;
                                                        revealed[a][b] = true;
                                                        // if the tile had no bomb neighbors (its a 0), start recursive
                                                        // method
                                                    } else if (count[a][b] == 0) {
                                                        reveal(a, b);
                                    ...
                                    // remove one mine from the counter (the one that was clicked) and increment
                                    // first click, so every click after this (mine or not) acts normally
                                    numMines--;
                                    firstClick++;
                    ...
                    buttons[i][j].addMouseListener(new MouseAdapter() {
                        ...
                        public void mousePressed(MouseEvent e) {
                            ...
                            if (SwingUtilities.isRightMouseButton(e)) {
                                ...
                                // the flaggedM counter is not incremented as a mine has not been flagged, win
                                // condition can only be met by correctly flagging mines
                                if (flagged[I][J] == true) {
                                    flagged[I][J] = false;
                                    if (clicked[I][J] < 1) {
                                        try {
                                ...
                                if (firstClick == 0) {
                                    ...
                                    firstClick = 1;
                                    // reveal this tiles neighbors, skipping mines, this avoids situations where
                                    // they only have one revealed tile, so they have no information to solve with
                                    for (int a = I - 1; a < I + 2; a++) {
                                        for (int b = J - 1; b < J + 2; b++) {
                                            try {
                                                if (minef[a][b] == 0 && count[a][b] > 0) {
                                    ...
                                    // if a tile is already revealed and is clicked again, all of its neighbors are
                                    // revealed, allowing for faster gameplay for player
                                    // if this tile has not already been clicked (or revealed by any other method)
                                    // then just reveal it
                                    if (clicked[I][J] == 0) {
                                        revealed[I][J] = true;
                                        ...
                                        for (int a = I - 1; a < I + 2; a++) {
                                            for (int b = J - 1; b < J + 2; b++) {
                                                try {
                                                    // if you click again and have not flagged the surrounding mines,
                                                    // reveal them and end in loss
                                                    if (flagged[a][b] == false) {
                                                        if (count[a][b] == 9) {
                                                            ...
                                                            // if the neighbor is empty and not flagged, reveal it
                                                            // (recur if 0)
                                                        ...
                                                        // if a neighbor is flagged, skip it, keeping it as flagged,
                                                        // mine or otherwise
